        ...
        switch (field.name.toLowerCase()) {
          case 'gitrepository' : {
            if ( this.currentSpace && (this.currentSpace.attributes.name || '' ).length > 0 ) {
              ...
              // handle the scenario when someone chages the name (that is defaulted to the space name) to something
              // else and the expectation that this defaults to the repo name. Do if that changes then default repo
              // name needs to change too !!
          ...
          case 'named' : {
            ...
            if ( field.display.note ) {
              ...
              field.display.note = field.display.note.replace(/application jar is/ig,
                                                              'Application generated assets are');
