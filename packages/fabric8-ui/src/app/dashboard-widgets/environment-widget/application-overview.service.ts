    ...
    return this.pollTimer.pipe(
      mergeMap(() =>
        this.deploymentApiService.getApplications(spaceId).pipe(
          ...
          map(
            (apps: Application[]): ApplicationAttributesOverview[] =>
              apps.map(
                (app: Application): ApplicationAttributesOverview => {
                  const appName: string = app.attributes.name;
                  const deploymentsInfo: DeploymentPreviewInfo[] = app.attributes.deployments
                    .map(
                    ...
                    .sort(
                      (a: DeploymentPreviewInfo, b: DeploymentPreviewInfo): number =>
