...
import { DeploymentStatusService, Status, StatusType } from './deployment-status.service';
...
describe('DeploymentStatusService', (): void => {
          ...
          DeploymentStatusService,
          {
            ...
            useFactory: (): jasmine.SpyObj<DeploymentsService> => {
              ...
              svc.getDeploymentCpuStat.and.returnValue(
                new BehaviorSubject<CpuStat[]>([{ used: 3, quota: 10 }]),
              );
              svc.getDeploymentMemoryStat.and.returnValue(
                new BehaviorSubject<MemoryStat[]>([{ used: 4, quota: 10, units: MemoryUnit.GB }]),
              );
              svc.getPods.and.returnValue(
                new BehaviorSubject<Pods>({ total: 1, pods: [[PodPhase.RUNNING, 1]] }),
              );
              svc.getEnvironmentCpuStat.and.returnValue(
                new BehaviorSubject<CpuStat>({ used: 3, quota: 10 }),
              );
              svc.getEnvironmentMemoryStat.and.returnValue(
                new BehaviorSubject<MemoryStat>({ used: 4, quota: 10, units: MemoryUnit.GB }),
              );
              return svc;
            },
  ...
  describe('#getDeploymentAggregateStatus', (): void => {
    it('should return OK status with no message when no stats are nearing quota', (done: DoneFn): void => {
      service
        .getDeploymentAggregateStatus('foo', 'bar', 'baz')
        .pipe(first())
        .subscribe(
          (status: Status): void => {
    ...
    it('should return combined status when multiple stats are nearing quota', (done: DoneFn): void => {
      ...
      deploymentsService
        ...
        .next([{ used: 9, quota: 10, units: MemoryUnit.MB }]);
      service
        .getDeploymentAggregateStatus('foo', 'bar', 'baz')
        .pipe(first())
        .subscribe(
          (status: Status): void => {
            ...
            expect(status.message).toEqual(
              'CPU usage is nearing capacity. Memory usage is nearing capacity.',
            );
            done();
          },
        );
    ...
    it('should return combined status when multiple stats are nearing or at quota', (done: DoneFn): void => {
      ...
      deploymentsService
        .getDeploymentMemoryStat()
        .next([{ used: 10, quota: 10, units: MemoryUnit.MB }]);
      service
        .getDeploymentAggregateStatus('foo', 'bar', 'baz')
        .pipe(first())
        .subscribe(
          (status: Status): void => {
            ...
            expect(status.message).toEqual(
              'CPU usage is nearing capacity. Memory usage has reached capacity.',
            );
            done();
          },
        );
  ...
  describe('#getDeploymentCpuStatus', (): void => {
    it('should correctly invoke the deployments service', (): void => {
      ...
      expect(deploymentsService.getPods).toHaveBeenCalledWith(
        spaceId,
        environmentName,
        applicationName,
      );
      expect(deploymentsService.getDeploymentCpuStat).toHaveBeenCalledWith(
        spaceId,
        environmentName,
        applicationName,
        1,
    ...
    it('should return OK status when not nearing quota', (done: DoneFn): void => {
      service
        .getDeploymentCpuStatus('foo', 'bar', 'baz')
        .pipe(first())
        .subscribe(
          (status: Status): void => {
            expect(status.type).toEqual(StatusType.OK);
  ...
  describe('#getDeploymentMemoryStatus', (): void => {
    it('should correctly invoke the deployments service', (): void => {
      ...
      expect(deploymentsService.getDeploymentMemoryStat).toHaveBeenCalledWith(
        spaceId,
        environmentName,
        applicationName,
        1,
    ...
    it('should return OK status when not nearing quota', (done: DoneFn): void => {
      service
        .getDeploymentMemoryStatus('foo', 'bar', 'baz')
        .pipe(first())
        .subscribe(
          (status: Status): void => {
            expect(status.type).toEqual(StatusType.OK);
      ...
      service
        ...
        .subscribe(
          (status: Status): void => {
            ...
            deploymentsService
              .getDeploymentMemoryStat()
              .next([{ used: 0, quota: 0, units: MemoryUnit.MB }]);
            service
              .getDeploymentMemoryStatus('foo', 'bar', 'baz')
              .pipe(first())
              .subscribe(
  ...
  describe('getEnvironmentCpuStatus', (): void => {
    it('should correctly invoke the deployments service', (): void => {
      ...
      expect(deploymentsService.getEnvironmentCpuStat).toHaveBeenCalledWith(
        spaceId,
        environmentName,
      );
    });
    ...
    it('should return OK status when not nearing quota', (done: DoneFn): void => {
      service
        .getEnvironmentCpuStatus('foo', 'bar')
        .pipe(first())
        .subscribe(
          (status: Status): void => {
            expect(status.type).toEqual(StatusType.OK);
  ...
  describe('getEnvironmentMemoryStatus', (): void => {
    it('should correctly invoke the deployments service', (): void => {
      ...
      expect(deploymentsService.getEnvironmentMemoryStat).toHaveBeenCalledWith(
        spaceId,
        environmentName,
      );
    });
    ...
    it('should return OK status when not nearing quota', (done: DoneFn): void => {
      service
        .getEnvironmentMemoryStatus('foo', 'bar')
        .pipe(first())
        .subscribe(
          (status: Status): void => {
            expect(status.type).toEqual(StatusType.OK);
    ...
    it('should return WARN status when nearing quota', (done: DoneFn): void => {
      deploymentsService
        ...
        .next({ used: 9, quota: 10, units: MemoryUnit.GB });
      service
        .getEnvironmentMemoryStatus('foo', 'bar')
        .pipe(first())
        .subscribe(
    ...
    it('should return ERR status when at quota', (done: DoneFn): void => {
      deploymentsService
        .getEnvironmentMemoryStat()
        .next({ used: 10, quota: 10, units: MemoryUnit.GB });
      service
        .getEnvironmentMemoryStatus('foo', 'bar')
        .pipe(first())
        .subscribe(
