...
describe('DeploymentsService', (): void => {
      ...
      TestBed.configureTestingModule({
        providers: [
          { provide: DeploymentApiService, useValue: createMock(DeploymentApiService) },
          {
            ...
            useValue: jasmine.createSpyObj<NotificationsService>('NotificationsService', [
              'message',
            ]),
          },
  ...
  describe('#getApplications', (): void => {
    it('should publish faked application names', (done: DoneFn): void => {
      apiService.getApplications.and.returnValue(
        of([
          {
            attributes: {
              name: 'vertx-hello',
  ...
  describe('#getEnvironments', (): void => {
    it('should sort environments', (done: DoneFn): void => {
      apiService.getEnvironments.and.returnValue(
        of([
          {
            attributes: {
              name: 'run',
    ...
    it('should return singleton array result', (done: DoneFn): void => {
      apiService.getEnvironments.and.returnValue(of([{ attributes: { name: 'stage' } }]));
      service
        .getEnvironments('foo-spaceId')
        .pipe(first())
        .subscribe(
  ...
  describe('#isApplicationDeployedInEnvironment', (): void => {
    it('should be true for included deployments', (done: DoneFn): void => {
      ...
      service
        .isApplicationDeployedInEnvironment('foo-spaceId', 'run', 'vertx-hello')
        .pipe(first())
        .subscribe(
          (deployed: boolean): void => {
    ...
    it('should be false for excluded deployments', (done: DoneFn): void => {
      ...
      service
        .isApplicationDeployedInEnvironment('foo-spaceId', 'stage', 'vertx-hello')
        .pipe(first())
        .subscribe(
          (deployed: boolean): void => {
  ...
  describe('#isDeployedInEnvironment', (): void => {
    it('should be true for included environments', (done: DoneFn): void => {
      ...
      service
        .isDeployedInEnvironment('foo-spaceId', 'run')
        .pipe(first())
        .subscribe(
          (deployed: boolean): void => {
            expect(deployed).toEqual(true);
    ...
    it('should be false for excluded environments', (done: DoneFn): void => {
      ...
      service
        .isDeployedInEnvironment('foo-spaceId', 'stage')
        .pipe(first())
        .subscribe(
          (deployed: boolean): void => {
            expect(deployed).toEqual(false);
  ...
  describe('#getVersion', (): void => {
    it('should return 1.0.2', (done: DoneFn): void => {
      ...
      service
        .getVersion('foo-spaceId', 'stage', 'vertx-hello')
        .pipe(first())
        .subscribe(
          (version: string): void => {
  ...
  describe('#canScale', (): void => {
    ...
    it('should return true when remaining quota is sufficient', (done: DoneFn): void => {
      ...
      apiService.getQuotaRequirementPerPod.and.returnValue(
        of({
          cpucores: 1,
          memory: 0.5 * GB,
        }),
      ...
      service
        .canScale('foo-spaceId', 'stage', 'vertx-hello')
        .pipe(first())
        .subscribe(
  ...
  describe('#getMaximumPods', () => {
    ...
    it('should return appropriate number of maximum pods for typical scenario', (done: DoneFn): void => {
      ...
      service
        .getMaximumPods('foo-spaceId', 'stage', 'vertx-hello')
        .pipe(first())
        .subscribe(
          (maxPods: number): void => {
  ...
  describe('#scalePods', () => {
    it('should return success message on success', (done: DoneFn): void => {
      ...
      service
        .scalePods('foo-spaceId', 'stage', 'vertx-hello', 2)
        .pipe(first())
        .subscribe(
          (msg: string) => {
  ...
  describe('#getPods', (): void => {
    it('should return pods for an existing deployment', (done: DoneFn): void => {
      ...
      service
        .getPods('foo-spaceId', 'stage', 'vertx-hello')
        .pipe(first())
        .subscribe(
          (pods: Pods): void => {
            expect(pods).toEqual({
              total: 2,
              pods: [[PodPhase.PENDING, 0], [PodPhase.RUNNING, 1], [PodPhase.TERMINATING, 1]],
            });
            done();
          },
        );
    ...
    it('should return pods when there are multiple deployments', (done: DoneFn): void => {
      ...
      service
        .getPods('foo-spaceId', 'run', 'vertx-hello')
        .pipe(first())
        .subscribe(
          (pods: Pods): void => {
            expect(pods).toEqual({
              total: 6,
              pods: [[PodPhase.PENDING, 2], [PodPhase.RUNNING, 3], [PodPhase.TERMINATING, 1]],
            } as Pods);
            done();
          },
        );
  ...
  describe('#getDeploymentCpuStat', (): void => {
    it('should combine timeseries and quota data', (done: DoneFn): void => {
      apiService.getTimeseriesData.and.returnValue(
        of({
      ...
      apiService.getLatestTimeseriesData.and.returnValue(
        of({
          cores: {
            time: 9,
            value: 9,
      ...
      service
        .getDeploymentCpuStat('foo-space', 'foo-env', 'foo-app', 3)
        .pipe(first())
        .subscribe(
          (stats: CpuStat[]): void => {
    ...
    it('should round usage data points', (done: DoneFn): void => {
      apiService.getTimeseriesData.and.returnValue(
        of({
          cores: [{ value: 0.0001, time: 1 }, { value: 0.00001, time: 2 }],
          memory: [{ value: 3, time: 3 }, { value: 4, time: 4 }],
          net_rx: [{ value: 5, time: 5 }, { value: 6, time: 6 }],
          net_tx: [{ value: 7, time: 7 }, { value: 8, time: 8 }],
          start: 1,
  ...
  describe('#getDeploymentMemoryStat', (): void => {
    it('should combine timeseries and quota data', (done: DoneFn): void => {
      ...
      service
        .getDeploymentMemoryStat('foo-space', 'foo-env', 'foo-app', 3)
        .pipe(first())
        .subscribe(
          (stats: MemoryStat[]): void => {
            expect(stats).toEqual([
    ...
    it('should scale results to the sample with greatest unit', (done: DoneFn): void => {
      apiService.getTimeseriesData.and.returnValue(
        of({
          cores: [{ value: 0, time: 0 }, { value: 0, time: 1 }],
          net_rx: [{ value: 0, time: 0 }, { value: 0, time: 1 }],
          net_tx: [{ value: 0, time: 0 }, { value: 0, time: 1 }],
          memory: [
  ...
  describe('#getDeploymentNetworkStat', (): void => {
    it('should return scaled timeseries data', (done: DoneFn): void => {
      ...
      service
        .getDeploymentNetworkStat('foo-space', 'foo-env', 'foo-app', 3)
        .pipe(first())
        .subscribe(
          (stats: NetworkStat[]): void => {
            expect(stats).toEqual([
    ...
    it('should scale results to the sample with greatest unit', (done: DoneFn): void => {
      apiService.getTimeseriesData.and.returnValue(
        of({
          ...
          memory: [{ value: 0, time: 0 }, { value: 0, time: 1 }],
          start: 0,
          end: 1,
        }),
      );
      apiService.getLatestTimeseriesData.and.returnValue(
        of({
          ...
          net_tx: {
            time: 2,
            value: 1 * Math.pow(1024, 3),
          },
          net_rx: {
            time: 2,
            value: 750 * Math.pow(1024, 2),
          },
          memory: {
            time: 0,
            value: 0,
  ...
  describe('#getEnvironmentCpuStat', (): void => {
    it('should return a "used" value of 8 and a "quota" value of 10', (done: DoneFn): void => {
      ...
      service
        .getEnvironmentCpuStat('foo-spaceId', 'stage')
        .pipe(first())
        .subscribe(
          (cpuStat: CpuStat): void => {
  ...
  describe('#getEnvironmentMemoryStat', (): void => {
    it('should return a "used" value of 512 and a "quota" value of 1024 with units in "MB"', (done: DoneFn): void => {
      ...
      service
        .getEnvironmentMemoryStat('foo-spaceId', 'stage')
        .pipe(first())
        .subscribe(
          (memoryStat: MemoryStat): void => {
  ...
  describe('#getEnvironmentCpuUtilization', (): void => {
    it('should return CPU utilization for specified and other spaces', (done: DoneFn): void => {
      const gb: number = Math.pow(1024, 3);
      apiService.getQuotas.and.returnValue(
        of([
          {
            attributes: {
              name: 'stage',
      ...
      service
        .getEnvironmentCpuUtilization('foo-spaceId', 'run')
        .pipe(first())
        .subscribe(
          (utilization: CpuResourceUtilization): void => {
  ...
  describe('#getEnvironmentMemoryUtilization', (): void => {
    it('should return Memory utilization for specified and other spaces', (done: DoneFn): void => {
      ...
      service
        .getEnvironmentMemoryUtilization('foo-spaceId', 'run')
        .pipe(first())
        .subscribe(
          (utilization: MemoryResourceUtilization): void => {
            expect(utilization).toEqual({
              currentSpaceUsage: ScaledMemoryStat.from(
                new ScaledMemoryStat(0, 1 * gb),
                MemoryUnit.MB,
              ),
              otherSpacesUsage: ScaledMemoryStat.from(
                new ScaledMemoryStat(0.5 * gb, 1 * gb),
                MemoryUnit.MB,
              ),
            });
            done();
  ...
  describe('#deleteDeployment', (): void => {
    it('should delete a deployment with the correct URL', (done: DoneFn): void => {
      ...
      service
        .deleteDeployment('spaceId', 'envId', 'appId')
        .pipe(first())
        .subscribe(
          (msg: string): void => {
            expect(msg).toEqual('Deployment has successfully deleted');
            expect(apiService.deleteDeployment).toHaveBeenCalledWith('spaceId', 'envId', 'appId');
            done();
          },
    ...
    it('should throw an error if it cannot delete', (done: DoneFn): void => {
      ...
      expect(apiService.deleteDeployment).not.toHaveBeenCalled();
      service
        .deleteDeployment(spaceId, environmentId, appId)
        .pipe(first())
        .subscribe(
          (): void => done.fail(),
          (err: string): void => {
            expect(err).toEqual(`Failed to delete ${appId} in ${spaceId} (${environmentId})`);
            expect(apiService.deleteDeployment).toHaveBeenCalledWith(spaceId, environmentId, appId);
            done();
          },
        );
    });
  ...
  describe('application links', (): void => {
    it('should provide logs URL', (done: DoneFn): void => {
      ...
      service
        .getLogsUrl('foo-space', 'foo-env', 'foo-app')
        .pipe(first())
        .subscribe(
          (url: string): void => {
    ...
    it('should provide console URL', (done: DoneFn): void => {
      ...
      service
        .getConsoleUrl('foo-space', 'foo-env', 'foo-app')
        .pipe(first())
        .subscribe(
          (url: string): void => {
    ...
    it('should provide application URL', (done: DoneFn): void => {
      ...
      service
        .getAppUrl('foo-space', 'foo-env', 'foo-app')
        .pipe(first())
        .subscribe(
          (url: string): void => {
  ...
  describe('#hasDeployments', (): void => {
    ...
    it('should return true if there are deployed applications', (done: DoneFn): void => {
      ...
      svc
        .hasDeployments('foo-spaceId', environments)
        .pipe(first())
        .subscribe(
          (bool: boolean): void => {
  ...
  describe('getApplications', (): void => {
    function testApplicationsError(status: number, expectedMessage: Notification): void {
      ...
      apiSvc.getApplications.and.returnValue(throwError(error, vs));

      const svc: DeploymentsService = TestBed.get(DeploymentsService);
      svc
        .getApplications('spaceId')
        .pipe(first())
        .subscribe(
  ...
  describe('getEnvironments', (): void => {
    function testEnvironmentsError(status: number, expectedMessage: Notification): void {
      ...
      apiSvc.getEnvironments.and.returnValue(throwError(error, vs));
  ...
  describe('getDeploymentCpuStat', (): void => {
    ...
    function testGetTimeSeriesError(status: number, expectedMessage: Notification): void {
      ...
      apiSvc.getTimeseriesData.and.returnValue(throwError(error));

      const svc: DeploymentsService = TestBed.get(DeploymentsService);

      svc
        .getDeploymentCpuStat('foo-space', 'foo-env', 'foo-app')
        .pipe(first())
        .subscribe(
          (): void => fail('should not emit'),
          (): void => fail('should not emit'),
    ...
    function testGetLatestTimeSeriesError(status: number, expectedMessage: Notification): void {
      ...
      apiSvc.getLatestTimeseriesData.and.returnValue(throwError(error));
    ...
    function testGetApplicationsError(status: number, expectedMessage: Notification): void {
      ...
      apiSvc.getApplications.and.returnValue(throwError(error));
